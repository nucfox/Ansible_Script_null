...
def check_container_stats(container_name,collect_item):
    ...
    if collect_item == 'cpu_total_usage':
        ...
        result=new_result['memory_stats']['stats']['total_rss']
        ...
        mem_usage=new_result['memory_stats']['stats']['total_rss']
        mem_limit=new_result['memory_stats']['limit']
        result=round(float(mem_usage)/float(mem_limit)*100.0,2)
    return result
if __name__ == "__main__":
    docker_client = docker.DockerClient(base_url='unix://var/run/docker.sock', version='1.26')
