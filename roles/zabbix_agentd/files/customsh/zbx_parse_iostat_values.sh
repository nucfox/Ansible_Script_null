#!/bin/bash
PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin:~/sbin:~/bin
source /etc/profile

_A=$1

_MOUNTarray=(`df -t ext4 | awk 'NR>1{print $NF}'`)

_DEVarray=(`df -t ext4 | awk 'NR>1{print $1}' | awk 'BEGIN{FS="/"}{print $NF}'`)


#echo ${_MOUNTarray[@]}
#echo ${_DEVarray[@]}
#echo $_A
for ((i=0;i<${#_MOUNTarray[@]};i++)){
        if [ "$_A" = "${_MOUNTarray[$i]}" ];then
#       if [ "$_A" = "/boot" ];then
                break;
#               echo $_A                
        fi
}
#echo $i
#echo ${_DEVarray[$i]}
_B=`echo ${_DEVarray[$i]} | cut -b 4`

expr $_B + 10 >> /dev/null 2>&1

if [ "$?" == "0" ];then
        _C=`echo ${_DEVarray[$i]} | cut -b 1-3`
else
        _C=`echo ${_DEVarray[$i]}`
fi

#echo $_C


dev=$_C

type=$2

#enable debug mode
debug=0

if [[ -z "$dev" ]]; then
  echo "error: wrong input value (device)"
  exit 1
fi

if [[ -z "$type" ]]; then
  echo "error: wrong input value (type)"
  exit 1
fi

columns=`iostat -xN |egrep -o "^Device.*"`

columnsarray=($columns)

column_id=1

for i in "${columnsarray[@]}"
do
        #echo "column: $i"

        if [[ "$i" = "$type" ]]; then

            if [[ $debug -eq 1 ]]; then
                echo "right column (${i}) found...column_id: $column_id "
            fi

            id="$"
            column_id_id=$id$column_id

            iostats=`iostat -xN 1 2 |egrep -o "^${dev}[[:space:]]+.*" |awk "NR>1{print ${column_id_id}}"`
        fi
    column_id=$[column_id + 1]
done

if [ -z "$iostats" ]; then
    echo "error: \"device\" or \"type\" not found (${dev},${type})"
    exit 3
fi

iostats_lines=`wc -l <<< "$iostats"`

if [ $iostats_lines -ne 1 ]; then
    echo "error: wrong output value (${iostats_lines})"
    exit 2
fi

echo $iostats

if [[ $debug -eq 1 ]]; then
    echo "- - - - - - - - - -"
    echo $columns
    iostats_debug=`iostat -xN |egrep -o "^${dev}[[:space:]]+.*"`
    echo $iostats_debug
    echo "- - - - - - - - - -"
fi

exit 0
